    ...
    queryFn: async (): Promise<DashboardStats> => {
      ...
      const [
        ...
        supabase
          .from("donations")
          .select("amount")
          .gte("donation_date", startOfDay.toISOString())
        ...
        supabase
          .from("donations")
          .select("amount")
          .gte("donation_date", startOfYear.toISOString())
          .eq("payment_status", "approved"),
        ...
        supabase
          ...
          .gte("aid_date", startOfMonth.toISOString()),
        // Active volunteers
        supabase
          .from("volunteers")
          .select("*", { count: "exact", head: true })
...
export function useMonthlyDonationTrend(months: number = 6) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MonthlyTrend[]> => {
      ...
      const { data, error } = await supabase.rpc("get_monthly_donation_trend", {
      ...
      donations?.forEach(
        (d: { donation_date?: string | null; amount?: number | null }) => {
          const monthKey =
            d.donation_date?.slice(0, 7) ||
            new Date().toISOString().slice(0, 7);
          monthlyTotals[monthKey] =
            (monthlyTotals[monthKey] || 0) + (d.amount || 0);
        },
      );
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CategoryDistribution[]> => {
      ...
      const cityMap = new Map(
        cities?.map((c: { id: string; name: string }) => [c.id, c.name]) || [],
      );
