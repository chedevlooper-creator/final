...
export function useUpdateSkillCategory() {
  ...
  return useMutation({
    mutationFn: async ({ id, values }: { id: string; values: UpdateSkillCategoryInput }) => {
      const { data, error } = await supabase
        .from('skill_categories')
        .update(values)
        .eq('id', id)
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['skill-categories', variables.id] })
      queryClient.invalidateQueries({ queryKey: ['skill-categories'] })
    },
  })
}
...
export function useSkills(filters?: SkillFilters) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.search) {
        query = query.or(`name.ilike.%${filters.search}%,description.ilike.%${filters.search}%,category_name.ilike.%${filters.search}%`)
      }
...
export function useUpdateSkill() {
  ...
  return useMutation({
    mutationFn: async ({ id, values }: { id: string; values: UpdateSkillInput }) => {
      const { data, error } = await supabase
        .from('skills')
        .update(values)
        .eq('id', id)
...
export function useCreateVolunteerSkill() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['volunteer-skills', variables.volunteer_id] })
      queryClient.invalidateQueries({ queryKey: ['volunteers', variables.volunteer_id] })
    },
  })
}
...
export function useUpdateVolunteerSkill() {
  ...
  return useMutation({
    mutationFn: async ({ id, values }: { id: string; values: UpdateVolunteerSkillInput }) => {
      const { data, error } = await supabase
        .from('volunteer_skills')
        .update(values)
        .eq('id', id)
