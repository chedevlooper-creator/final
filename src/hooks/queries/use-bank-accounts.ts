...
export function useCreateBankAccount() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['bank-accounts', variables.needy_person_id] })
    },
  })
}
...
export function useUpdateBankAccount() {
  ...
  return useMutation({
    mutationFn: async ({ id, values }: { id: string; values: Partial<BankAccountFormValues> }) => {
      const { data, error } = await supabase
        .from('needy_bank_accounts')
        .update(values)
        .eq('id', id)
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['bank-accounts', data.needy_person_id] })
    },
  })
}
...
export function useDeleteBankAccount() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['bank-accounts', result.needyPersonId] })
    },
  })
}
