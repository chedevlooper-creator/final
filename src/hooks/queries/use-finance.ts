...
export function useFinanceTransactions(filters?: FinanceTransactionFilters) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return { data: data as FinanceTransaction[], count: count || 0, page, limit }
        },
    })
}
...
export function useFinanceSummary() {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const [allIncomes, allExpenses, monthlyIncomes, monthlyExpenses, cashTrans, bankTrans] = await Promise.all([
                supabase.from('finance_transactions').select('amount').eq('type', 'income').eq('status', 'completed'),
                supabase.from('finance_transactions').select('amount').eq('type', 'expense').eq('status', 'completed'),
                supabase.from('finance_transactions').select('amount').eq('type', 'income').eq('status', 'completed').gte('created_at', startOfMonth.toISOString()),
                supabase.from('finance_transactions').select('amount').eq('type', 'expense').eq('status', 'completed').gte('created_at', startOfMonth.toISOString()),
                supabase.from('finance_transactions').select('amount, type').eq('account_type', 'cash').eq('status', 'completed'),
                supabase.from('finance_transactions').select('amount, type').eq('account_type', 'bank').eq('status', 'completed'),
            ])

            const sumAmounts = (data: { amount: number }[] | null) => 
                data?.reduce((sum, t) => sum + (t.amount || 0), 0) || 0

            const calculateBalance = (data: { amount: number; type: string }[] | null) => {
                if (!data) return 0
...
export function useUpdateFinanceTransaction() {
    ...
    return useMutation({
        mutationFn: async ({ id, values }: { id: string; values: Partial<FinanceTransaction> }) => {
            const { data, error } = await supabase
                .from('finance_transactions')
        ...
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: ['finance-transactions', variables.id] })
            queryClient.invalidateQueries({ queryKey: ['finance-transactions'] })
            queryClient.invalidateQueries({ queryKey: ['finance-summary'] })
        },
    })
...
export function useCashTransactions(filters?: Omit<FinanceTransactionFilters, 'account_type'>) {
    return useFinanceTransactions({ ...filters, account_type: 'cash' })
}

export function useBankTransactions(filters?: Omit<FinanceTransactionFilters, 'account_type'>) {
