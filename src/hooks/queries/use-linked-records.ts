...
export function useUpdateLinkedRecord<T extends { needy_person_id?: string }>(
  table: LinkedRecordTable,
) {
  const queryClient = useQueryClient();
  const supabase = createClient();
...
export function useNeedyDependents(needyPersonId: string, isActive?: boolean) {
  return useLinkedRecords<NeedyDependent>("needy_dependents", needyPersonId, {
    isActive,
  });
}
...
export function useNeedyInterviews(needyPersonId: string) {
  return useLinkedRecords<NeedyInterview>("needy_interviews", needyPersonId, {
    orderBy: "interview_date",
  });
}

export function useNeedyAidsReceived(needyPersonId: string) {
  return useLinkedRecords<NeedyAidReceived>(
    "needy_aids_received",
    needyPersonId,
    { orderBy: "aid_date" },
  );
...
export function useNeedySocialCards(needyPersonId: string, isActive?: boolean) {
  return useLinkedRecords<NeedySocialCard>(
    "needy_social_cards",
    needyPersonId,
    { isActive },
  );
