...
export function DependentsTab({ needyPersonId, onClose }: DependentsTabProps) {
  ...
  const { data: dependents = [], isLoading } = useLinkedRecords<NeedyDependent>('needy_dependents', needyPersonId)
  const createMutation = useCreateLinkedRecord<NeedyDependent>('needy_dependents')
  const updateMutation = useUpdateLinkedRecord<NeedyDependent>('needy_dependents')
  ...
  const filteredDependents = dependents.filter(dep => {
    const matchesSearch = !searchValue || 
      `${dep.first_name} ${dep.last_name}`.toLowerCase().includes(searchValue.toLowerCase()) ||
            ...
            <TableBody>
              {filteredDependents.length === 0 ? (
                ...
                filteredDependents.map((dep) => (
                  <TableRow key={dep.id}>
                    <TableCell></TableCell>
                    <TableCell>{dep.first_name} {dep.last_name}</TableCell>
                    <TableCell>{dep.identity_number}</TableCell>
                    <TableCell>
                      {DEPENDENT_RELATION_TYPE_OPTIONS.find(t => t.value === dep.relation_type)?.label}
                    </TableCell>
                    <TableCell>-</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDelete(dep.id)}>
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setFormData({ ...formData, relation_type: v as DependentRelationType })}
              >
                <SelectTrigger>
