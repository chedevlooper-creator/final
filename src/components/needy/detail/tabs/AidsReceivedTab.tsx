...
export function AidsReceivedTab({
  needyPersonId,
  onClose,
}: AidsReceivedTabProps) {
  const [searchValue, setSearchValue] = useState("");
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);

  const { data: aids = [], isLoading } = useLinkedRecords<NeedyAidReceived>(
    "needy_aids_received",
    needyPersonId,
    { orderBy: "aid_date" },
  );
  const createMutation = useCreateLinkedRecord<NeedyAidReceived>(
    "needy_aids_received",
  );
  ...
  const filteredAids = aids.filter(
    (aid) =>
      !searchValue ||
              ...
              {filteredAids.length === 0 ? (
                ...
                filteredAids.map((aid) => (
                  <TableRow key={aid.id}>
                    ...
                    <TableCell>
                      {aid.aid_date &&
                        format(new Date(aid.aid_date), "dd.MM.yyyy", {
                          locale: tr,
                        })}
                    </TableCell>
                    <TableCell>
                      {
                        AID_TYPE_OPTIONS.find((t) => t.value === aid.aid_type)
                          ?.label
                      }
                    </TableCell>
                    ...
                    <TableCell className="font-medium">
                      {aid.amount?.toLocaleString("tr-TR")} {aid.currency}
                    </TableCell>
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-1 rounded flex items-center gap-1 w-fit ${
                          aid.delivery_status === "delivered"
                            ? "bg-green-100 text-green-700"
                            : aid.delivery_status === "pending"
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              İhtiyaç sahibine yapılan yardımın detaylarını buradan
              kaydedebilirsiniz.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, aid_date: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setFormData({ ...formData, aid_type: v as AidType })
                  }
                >
                  <SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, aid_category: e.target.value })
                }
              />
            </div>
            <div>
