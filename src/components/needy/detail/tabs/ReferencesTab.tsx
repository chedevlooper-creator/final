...
import { Pencil, Trash2, Phone, CheckCircle2 } from 'lucide-react'
...
export function ReferencesTab(_props: ReferencesTabProps) {
  const [searchValue, setSearchValue] = useState('')
  const [isAddModalOpen, setIsAddModalOpen] = useState(false)
  const [references, _setReferences] = useState<Reference[]>([])
  ...
  const handleAdd = () => {
    setFormData({ reference_name: '', reference_type: '', relation: '', phone: '', email: '' })
          ...
          <Table>
            ...
            <TableBody>
              {references.length === 0 ? (
                ...
                references.map((ref) => (
                  <TableRow key={ref.id}>
                    <TableCell><Button variant="ghost" size="icon" className="h-8 w-8"><Phone className="h-4 w-4" /></Button></TableCell>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Tür</Label>
                <Select value={formData.reference_type} onValueChange={(v) => setFormData({ ...formData, reference_type: v as ReferenceType })}>
                  <SelectTrigger><SelectValue placeholder="Seçin" /></SelectTrigger>
                  <SelectContent>{REFERENCE_TYPE_OPTIONS.map((opt) => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div><Label>Yakınlık</Label><Input value={formData.relation} onChange={(e) => setFormData({ ...formData, relation: e.target.value })} /></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Telefon</Label><Input value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} /></div>
              <div><Label>E-posta</Label><Input value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} /></div>
            </div>
          </div>
          <DialogFooter>
