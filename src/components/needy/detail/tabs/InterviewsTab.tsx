              ...
              {interviews.length === 0 ? (
                ...
                interviews.map((int) => (
                  <TableRow key={int.id}>
                    ...
                    <TableCell>
                      {
                        INTERVIEW_OUTCOME_OPTIONS.find(
                          (o) => o.value === int.outcome,
                        )?.label
                      }
                    </TableCell>
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-1 rounded ${int.status === "completed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
                        {
                          INTERVIEW_STATUS_OPTIONS.find(
                            (s) => s.value === int.status,
                          )?.label
                        }
                      </span>
                    </TableCell>
          ...
          <div className="space-y-4 py-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setFormData({
                      ...formData,
                      interview_type: v as InterviewType,
                    })
                  }
                >
                  <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFormData({ ...formData, outcome: v as InterviewOutcome })
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="Seçin" />
                ...
                <SelectContent>
                  {INTERVIEW_OUTCOME_OPTIONS.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value}>
                      {opt.label}
                    </SelectItem>
                  ))}
