...
import {
  SocialCard,
  SOCIAL_CARD_TYPE_OPTIONS,
  SOCIAL_CARD_STATUS_OPTIONS,
  SocialCardType,
  SocialCardStatus,
  StatusFilter,
            ...
            <TableBody>
              {cards.length === 0 ? (
                ...
                cards.map((card) => (
                  <TableRow
                    key={card.id}
                    className={isExpired(card.expiry_date) ? "bg-red-50" : ""}
                  >
                    <TableCell>
                      <div
                        className={`p-2 rounded ${
                          card.card_type === "green_card"
                            ? "bg-green-100"
                            : card.card_type === "refugee_card"
                              ? "bg-blue-100"
                              : card.card_type === "disability_card"
                                ? "bg-purple-100"
                                : "bg-gray-100"
                        }`}
                      >
                    ...
                    <TableCell>
                      {card.issue_date &&
                        format(new Date(card.issue_date), "dd.MM.yyyy", {
                          locale: tr,
                        })}
                    </TableCell>
                    <TableCell
                      className={
                        isExpired(card.expiry_date)
                          ? "text-red-600 font-medium"
                          : ""
                      }
                    >
                      {card.expiry_date
                        ? format(new Date(card.expiry_date), "dd.MM.yyyy", {
                            locale: tr,
                          })
                        : "Süresiz"}
                    </TableCell>
                    <TableCell>
                      ...
                      >
                        {
                          SOCIAL_CARD_STATUS_OPTIONS.find(
                            (s) => s.value === card.status,
                          )?.label
                        }
                      </span>
                    </TableCell>
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFormData({ ...formData, card_type: v as SocialCardType })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {SOCIAL_CARD_TYPE_OPTIONS.map((opt) => (
