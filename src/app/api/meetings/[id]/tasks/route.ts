...
export async function POST(
  ...
  try {
    ...
    if (!title || title.trim().length < 3) {
      return NextResponse.json({ error: 'Görev başlığı en az 3 karakter olmalıdır' }, { status: 400 });
    }
    
    if (!assigned_to) {
      return NextResponse.json({ error: 'Görev atanmış bir personel seçmelisiniz' }, { status: 400 });
    }
