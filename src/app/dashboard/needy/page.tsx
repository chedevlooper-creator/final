...
const AddNeedyModal = dynamic(
  () =>
    import("@/components/needy/AddNeedyModal").then((mod) => ({
      default: mod.AddNeedyModal,
    })),
  {
    loading: () => (
...
export default function NeedyListPage() {
  ...
  const columns: ColumnDef<NeedyPerson>[] = [
    ...
    },
    {
      accessorKey: "type",
      header: "Tür",
      cell: ({ row }) => <span className="text-sm">İhtiyaç Sahibi Kişi</span>,
    },
    {
    ...
    },
    {
      ...
      header: "Kişi",
      cell: ({ row }) => (
        <span className="text-sm">{row.original.family_size || "-"}</span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <Link
              href={`/dashboard/needy/${row.original.id}?edit=true`}
              prefetch={true}
            >
              <DropdownMenuItem>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 p-3 bg-slate-50 rounded-lg border">
        ...
        <div className="ml-auto flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>|</span>
          <span>
            {page + 1} / {Math.ceil((data?.count || 0) / 20) || 1}
          </span>
        </div>
      </div>
