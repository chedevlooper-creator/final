...
import { Gift, ArrowLeft, Save, User } from "lucide-react";
...
export default function SacrificeDetailPage() {
  ...
  const statusConfig = SACRIFICE_STATUSES.find(
    (s) => s.value === sacrifice.sacrifice_status,
  );
  const assignedShares = sacrifice.shares.filter(
    (s) => s.status === "assigned",
  ).length;
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold">
                Kurban #{sacrifice.sacrifice_number}
              </h1>
            ...
            <p className="text-sm text-slate-500">
              {
                SACRIFICE_TYPES.find(
                  (t) => t.value === sacrifice.sacrifice_type,
                )?.label
              }{" "}
              - {sacrifice.share_count} Hisse
            </p>
          </div>
        </div>
        <Button
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-green-600">
                ₺{sacrifice.amount.toLocaleString("tr-TR")}
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-blue-600">
                {assignedShares}/{sacrifice.share_count}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-lg font-medium">
                {format(new Date(sacrifice.created_at), "dd.MM.yyyy", {
                  locale: tr,
                })}
              </p>
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Hisseler ({assignedShares}/{sacrifice.share_count})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
