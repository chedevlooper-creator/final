...
import { Stethoscope, Plus, Search, Eye, Hospital } from "lucide-react";
...
export default function ServicePage() {
  ...
  const filteredServices = mockServices.filter((s) => {
    ...
    if (
      search &&
      !s.needy_person.name.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
            ...
            <TableBody>
              {filteredServices.length === 0 ? (
                ...
                filteredServices.map((service) => {
                  ...
                  const serviceConfig =
                    SERVICE_TYPE_CONFIG[service.service_type];
                  return (
                    <TableRow key={service.id}>
                      ...
                      <TableCell className="font-mono text-sm">
                        {service.referral_number}
                      </TableCell>
                      <TableCell>
                        <div>
                          <p className="font-medium">
                            {service.needy_person.name}
                          </p>
                          <p className="text-xs text-slate-500">
                            {service.needy_person.phone}
                          </p>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-sm text-slate-500">
                        {service.appointment_date
                          ? format(
                              new Date(service.appointment_date),
                              "dd.MM.yyyy HH:mm",
                              { locale: tr },
                            )
                          : "-"}
