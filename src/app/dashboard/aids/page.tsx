...
export default function AidsListPage() {
  ...
  const columns: ColumnDef<Aid>[] = [
      ...
      cell: ({ row }) => (
        <div>
          <p className="font-medium">
            {row.original.needy_person?.first_name}{" "}
            {row.original.needy_person?.last_name}
          </p>
    ...
    },
    {
      ...
      header: "Yardım Türü",
      cell: ({ row }) => {
        const typeConfig = AID_TYPE_OPTIONS.find(
          (t) => t.value === row.original.aid_type,
        );
    ...
    },
    {
      ...
      cell: ({ row }) => {
        try {
          const date = row.original.aid_date
            ? new Date(row.original.aid_date)
            : null;
          return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        <Select
          value={aidType || "all"}
          onValueChange={(v) => setAidType(v === "all" ? "" : v)}
        >
        ...
        <Select
          ...
          onValueChange={(v) => setDeliveryStatus(v === "all" ? "" : v)}
        >
          <SelectTrigger className="w-48">
