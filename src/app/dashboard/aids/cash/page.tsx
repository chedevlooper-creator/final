...
import { Banknote, Plus, Search, Eye } from "lucide-react";
...
export default function CashAidsPage() {
  ...
  const filteredAids = mockCashAids.filter((a) => {
    ...
    if (
      search &&
      !a.needy_person.name.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  const deliveredCount = mockCashAids.filter(
                ...
                filteredAids.map((aid) => (
                  <TableRow key={aid.id}>
                    ...
                    <TableCell>
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {aid.needy_person.phone}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline">
                        {AID_TYPE_CONFIG[aid.aid_type]}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-sm text-slate-500">
                      {aid.delivery_date
                        ? format(new Date(aid.delivery_date), "dd.MM.yyyy", {
                            locale: tr,
                          })
                        : "-"}
                    </TableCell>
