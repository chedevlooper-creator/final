...
// Inner component that receives application as prop to avoid setState in useEffect
function ApplicationForm({
  application,
  updateMutation,
}: {
  application: Application
  updateMutation: ReturnType<typeof useUpdateApplication>
}) {
  // Initialize state from application props - key prop handles synchronization
  const [status, setStatus] = useState(application.status || '')
  const [priority, setPriority] = useState(application.priority || '')
  const [notes, setNotes] = useState(application.notes || '')
  const [approvedAmount, setApprovedAmount] = useState<string>(application.approved_amount?.toString() || '')
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold">
                Başvuru #{application.application_number || application.id}
              </h1>
...
export default function ApplicationDetailPage() {
  ...
  // Use key prop to remount the form when application changes
  return <ApplicationForm key={application.id} application={application} updateMutation={updateMutation} />
}
