...
import { Banknote, MoreHorizontal, Eye, Pencil, Receipt, X, Plus } from 'lucide-react'
      ...
      <div className="space-y-6">
        ...
        {!isLoading && data?.data?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1">Nakit Bağış Bulunmuyor</h3>
            <p className="text-sm text-muted-foreground">
              {hasActiveFilters ? 'Filtre kriterlerinize uygun nakit bağış bulunamadı.' : 'Henüz nakit bağış kaydı oluşturulmadı.'}
            </p>
          </div>
        )}
        ...
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle>Yeni Nakit Bağış</DialogTitle>
              <DialogDescription>
                Yeni bir nakit bağış kaydı oluşturun.
              </DialogDescription>
            </DialogHeader>
            <DonationForm onSuccess={() => setIsFormOpen(false)} defaultDonationType="cash" />
          </DialogContent>
        </Dialog>
      </div>
    </PageTransition>
