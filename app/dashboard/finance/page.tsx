...
export default function FinancePage() {
  ...
  const quickActions = [
    { title: 'Kasa İşlemleri', href: '/dashboard/finance/cash', icon: Wallet, color: 'bg-icon-blue' },
    { title: 'Banka İşlemleri', href: '/dashboard/finance/bank', icon: CreditCard, color: 'bg-icon-green' },
    { title: 'Raporlar', href: '/dashboard/finance/reports', icon: TrendingUp, color: 'bg-icon-purple' },
  ]
            ...
            <Card key={idx}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`p-3 rounded-lg ${stat.trend === 'up' ? 'bg-success/10' : 'bg-danger/10'}`}>
                    <Icon className={`h-6 w-6 ${stat.trend === 'up' ? 'text-success' : 'text-danger'}`} />
                  </div>
                </div>
              </CardContent>
            </Card>
